...
Map<BigInt, Operator> operators = {
  ...
  keywords['sha256']!: (args) {
    ...
    return mallocCost(
        Output(Program.fromAtom(sha256.convert(bytes).bytes), cost));
  },
  keywords['+']!: (args) {
  ...
  keywords['>s']!: (args) {
    ...
    final cost = Cost.grsBaseCost +
        (BigInt.from(list[0].atom.length) + BigInt.from(list[1].atom.length)) *
            Cost.grsCostPerByte;
    return Output(
        Program.fromBool(list[0].atomHex().compareTo(list[1].atomHex()) == 1),
        cost);
  },
  keywords['pubkey_for_exp']!: (args) {
    ...
    return mallocCost(
        Output(Program.fromAtom(exponent.getG1().toBytes()), cost));
  },
  keywords['point_add']!: (args) {
    ...
    return mallocCost(Output(Program.fromAtom(p.toBytes()), cost));
  },
  ...
  keywords['substr']!: (args) {
    ...
    return Output(Program.fromAtom(str.sublist(from, to)), BigInt.one);
  },
  keywords['concat']!: (args) {
    ...
    return mallocCost(Output(Program.fromAtom(bytes), cost));
  },
  keywords['ash']!: (args) {
