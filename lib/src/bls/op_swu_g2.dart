...
JacobianPoint osswu2Help(Fq2 t) {
  ...
  for (final root in rootsOfUnity) {
    ...
    if (y0.pow(BigInt.two) * gx0_den == gx0_num) {
      ...
      return JacobianPoint(x0_num * x0_den, y0 * x0_den.pow(BigInt.from(3)), x0_den, false,
          ec: defaultEcTwist,);
    }
  }
  ...
  for (final eta in etas) {
    ...
    if (y1.pow(BigInt.two) * gx1_den == gx1_num) {
      ...
      return JacobianPoint(x1_num * x1_den, y1 * x1_den.pow(BigInt.from(3)), x1_den, false,
          ec: defaultEcTwist,);
    }
  }
