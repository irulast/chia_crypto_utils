...
class ExchangeOfferService {
  ...
  Future<ChiaBaseResponse> sendMessageCoin({
    ...
    final cancelCoin = (await fullNode.getCoinsByPuzzleHashes(
            [messagePuzzlehash], includeSpentCoins: true))
        .where((coin) =>
            coin.parentCoinInfo == initializationCoinId && coin.amount == 3);
      ...
      if (offerAcceptFile.acceptedOfferHash ==
        ...
        if (offerAcceptFile.type == CrossChainOfferFileType.xchToBtcAccept) {
          ...
          if (!validateLightningPaymentRequest(
              lightningPaymentRequest, satoshis!)) return null;
        }
  ...
  Future<Coin> getCancelCoin(
      Coin initializationCoin, Puzzlehash messagePuzzlehash) async {
    try {
