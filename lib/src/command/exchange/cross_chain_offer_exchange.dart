...
late final ChiaFullNodeInterface _fullNode;
final _xchToBtcService = XchToBtcService();
final _btcToXchService = BtcToXchService();
final _crossChainOfferService = CrossChainOfferService(_fullNode);
...
Future<void> makeCrossChainOffer(ChiaFullNodeInterface fullNodeFromUrl) async {
  ...
  if (choice == '1') {
    ...
    offerFile = _crossChainOfferService.createBtcToXchOfferFile(
...
Future<void> acceptCrossChainOffer(ChiaFullNodeInterface fullNodeFromUrl) async {
  ...
  if (offerFile.type == CrossChainOfferFileType.btcToXch) {
    ...
    offerAcceptFile = _crossChainOfferService.createXchToBtcAcceptFile(
    ...
    offerAcceptFile = _crossChainOfferService.createBtcToXchAcceptFile(
      serializedOfferFile: serializedOfferFile,
      validityTime: validityTime,
      requestorPublicKey: requestorPublicKey,
    );
    ...
    if (choice == '1') {
      ...
      while (coins.totalValue < 100) {
        ...
        if (transactionValidated == false) {
          final mempoolItemsResponse = await _fullNode.getAllMempoolItems();
        ...
        coins = await _fullNode.getCoinsByPuzzleHashes(
          [coinPuzzlehash],
        );
      }
      ...
      final parentCoin = await _fullNode.getCoinById(coins[0].parentCoinInfo);
      final changePuzzlehash = parentCoin!.puzzlehash;

      final messageSpendBundle = _standardWalletService.createSpendBundle(
        payments: [
      ...
      await _fullNode.pushTransaction(messageSpendBundle);
...
Future<String?> waitForMessageCoin(
  ...
  while (true) {
    ...
    if (serializedCrossChainOfferFile.startsWith('ccoffer_accept')) {
      ...
      // the message address
      final verification = await _crossChainOfferService.verifyMessageCoinReceipt(
        messagePuzzlehash,
      ...
      final offerAcceptFileMemo =
          await _crossChainOfferService.getOfferAcceptFileFromMessagePuzzlehash(
        messagePuzzlehash,
        serializedCrossChainOfferFile,
      );
...
Future<List<Coin>> waitForEscrowCoins({
  ...
  while (escrowCoins.totalValue < amount) {
    ...
    escrowCoins = await _fullNode.getCoinsByPuzzleHashes(
      [escrowPuzzlehash],
    );
  }
...
Future<void> completeBtcToXchExchange(
  ...
  final sweepSpendBundle = _btcToXchService.createSweepSpendBundle(
  ...
  try {
    await _fullNode.pushTransaction(sweepSpendBundle);
    await verifyTransaction(escrowCoins, sweepPuzzlehash, _fullNode);
  } catch (e) {
...
Future<void> completeXchToBtcExchange(
  ...
  final clawbackSpendBundle = _xchToBtcService.createClawbackSpendBundle(
