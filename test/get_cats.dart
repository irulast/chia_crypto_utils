...
void main() async {
  ...
  final parentInnerPuzzle = uncurry(parentCoinSpend.puzzleReveal).arguments[2];
  ...
  final catPuzzle = CC_MOD.curry([
    ...
    // Program.fromBytes(walletVector!.puzzlehash.bytes),
    innerPuzzle
  ]);

  assert(const HexEncoder().convert(catPuzzle.hash()) == catCoin.puzzlehash.hex);
  ...
  final innerSolution = WalletService.makeSolutionFromConditions(conditions);
