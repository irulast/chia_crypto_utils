...
void main() {
  ...
  const offerValidityTime = 1671649043;

  const exchangeValidityTime = 3600;
  ...
  test('should correctly serialize and deserialize XCH to BTC offer file', () {
    const offeredAmount =
        ExchangeAmount(type: ExchangeAmountType.XCH, amount: 1000);

    const requestedAmount =
        ExchangeAmount(type: ExchangeAmountType.BTC, amount: 1);
    ...
    expect(deserializedOfferFile.initializationCoinId,
    ...
    expect(deserializedOfferFile.validityTime, equals(offerValidityTime));
  ...
  test('should correctly serialize and deserialize BTC to XCH offer file', () {
    ...
    final offerFile = BtcToXchMakerOfferFile(
      initializationCoinId: initializationCoinId,
      offeredAmount: offeredAmount,
      requestedAmount: requestedAmount,
      messageAddress: messageAddress,
  ...
  test(
      'should correctly serialize and deserialize XCH to BTC offer accept file',
      () {
    const offerFile =
    ...
    final offerAcceptFile = XchToBtcTakerOfferFile(
      initializationCoinId: initializationCoinId,
      validityTime: offerValidityTime,
      publicKey: publicKey,
      lightningPaymentRequest: decodedPaymentRequest,
    ...
    final serializedOfferFile = offerAcceptFile.serialize(privateKey);
    final deserializedTakerOfferFile =
        XchToBtcTakerOfferFile.fromSerializedOfferFile(serializedOfferFile);

    expect(deserializedTakerOfferFile.initializationCoinId,
        equals(initializationCoinId));
    expect(deserializedTakerOfferFile.validityTime, equals(offerValidityTime));
    expect(deserializedTakerOfferFile.publicKey, equals(publicKey));
    expect(
      deserializedTakerOfferFile.lightningPaymentRequest.paymentRequest,
      decodedPaymentRequest.paymentRequest,
    );
    expect(deserializedTakerOfferFile.acceptedOfferHash,
        equals(acceptedOfferHash));
  });

  test(
      'should correctly serialize and deserialize BTC to XCH offer accept file',
      () {
    const offerFile =
    ...
    final offerAcceptFile = BtcToXchTakerOfferFile(
      initializationCoinId: initializationCoinId,
    ...
    final serializedOfferFile = offerAcceptFile.serialize(privateKey);
    final deserializedTakerOfferFile =
        BtcToXchTakerOfferFile.fromSerializedOfferFile(serializedOfferFile);

    expect(deserializedTakerOfferFile.initializationCoinId,
        equals(initializationCoinId));
    expect(
        deserializedTakerOfferFile.validityTime, equals(exchangeValidityTime));
    expect(deserializedTakerOfferFile.publicKey, equals(publicKey));
    expect(deserializedTakerOfferFile.acceptedOfferHash,
        equals(acceptedOfferHash));
  });
  ...
  test('should throw exception when prefix is wrong', () {
    ...
    expect(
      ...
      throwsA(isA<InvalidCrossChainOfferFile>()),
    );
  });

  test(
      'should throw exception when trying to sign offer with wrong private key',
      () {
    final wrongPrivateKey = PrivateKey.fromHex(
        '076dc9996037449beaf87d5f1aa5287d392585e175a916e1b15a30b18b7e933c');
    ...
    expect(
      () => {offerFile.serialize(wrongPrivateKey)},
  ...
  test('should throw exception on expired offer file', () {
    ...
    const expiredValidityTime = 1571649043;
    ...
    expect(
      () => {CrossChainOfferFileService.checkValidity(offerFile)},
