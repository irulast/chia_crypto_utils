...
void main() {
  test('should determine if condition is reserve fee condition', () {
    ...
    final aggSigMeCondition = AggSigMeCondition(
      JacobianPoint.generateG1(),
      Bytes.encodeFromString('yo'),
    );

    final reserveFeeCondition = ReserveFeeCondition(5);

    expect(
      ReserveFeeCondition.isThisCondition(reserveFeeCondition.toProgram()),
      true,
    );
    expect(
      ReserveFeeCondition.isThisCondition(aggSigMeCondition.toProgram()),
      false,
    );
    expect(
      ReserveFeeCondition.isThisCondition(createCoinCondition.toProgram()),
      false,
    );
