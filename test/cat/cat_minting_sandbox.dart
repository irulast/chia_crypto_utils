...
void main() async {
  ...
  final coin = coins.firstWhere((element) => element.parentCoinInfo.toHex() == 'e3b0c44298fc1c149afbf4c8996fb92400000000000000000000000000000001');

  final publicKeyHex = 'add4758d972b7c2bd84798749ee2094c0c9e52b5b6618c985d4a8e841bf464a4079efa01e372d2307b6c26e6d1cceae6';
  final signature = JacobianPoint.fromBytesG2([143, 183, 25, 61, 234, 38, 181, 219, 45, 17, 171, 241, 53, 235, 128, 136, 232, 15, 113, 86, 83, 106, 74, 18, 11, 201, 65, 80, 224, 108, 120, 134, 90, 23, 69, 146, 10, 15, 87, 254, 179, 250, 196, 101, 6, 20, 21, 91, 21, 44, 114, 26, 33, 38, 56, 53, 179, 161, 239, 213, 7, 59, 53, 172, 245, 159, 0, 48, 71, 28, 248, 179, 78, 243, 54, 136, 195, 53, 248, 177, 88, 201, 33, 139, 187, 163, 251, 23, 134, 130, 49, 95, 173, 168, 86, 196]);

  final curriedTail = delegatedTailProgram.curry([Program.fromHex(publicKeyHex)]);
  // print('curried tail');
  // print(curriedTail.serializeHex());


  final curriedGenesisByCoinId = genesisByCoinIdProgram.curry([Program.fromBytes(coin.id.toUint8List())]);
  final tailSolution = Program.list([curriedGenesisByCoinId, Program.nil]);
  // print('tail solution');
  // print(tailSolution.serializeHex());
  ...
  final signedSpendBundle = walletService.createSpendBundle([coin], amount, Puzzlehash(catPuzzle.hash()), address.toPuzzlehash(), keychain);
  ...
  final eveCoin = Coin(
    confirmedBlockIndex: 111, 
    spentBlockIndex: 0, 
    coinbase: false, 
    timestamp: 1010, 
    parentCoinInfo: eveCoinPrototype.parentCoinInfo,
    puzzlehash: eveCoinPrototype.puzzlehash, 
    amount: eveCoinPrototype.amount,
  );


  final eveCat = CatCoin.fromCoin(eveCoin, eveParentCoinSpend, isEveCoin: true, assetId: Puzzlehash(curriedTail.hash()));
