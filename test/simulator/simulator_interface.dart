import 'dart:convert';

import 'package:chia_utils/chia_crypto_utils.dart';
import 'package:chia_utils/src/api/chia_full_node_interface.dart';
import 'package:chia_utils/src/api/exceptions/double_spend_exception.dart';
import 'package:chia_utils/src/cat/models/cat_coin.dart';
import 'package:chia_utils/src/core/models/models.dart';
import 'package:path/path.dart' as path;

import '../../lib/src/api/simulator_http_rpc.dart';

class SimulatorInterface {
  SimulatorInterface({String simulatorUrl =  'https://localhost:5000'})
    : simulatorHttpRpc = SimulatorHttpRpc(simulatorUrl,
        certPath: path.join(path.current, 'test/simulator/temp/config/ssl/full_node/private_full_node.crt'),
        keyPath: path.join(path.current, 'test/simulator/temp/config/ssl/full_node/private_full_node.key'),
      );

  static const testAddress = Address('xch1pdar6hnj8c9sgm74r72u40ed8cnpduzan5vr86qkvpftg0v52jkstxap9z');
  static const otherAddress = Address('xch1ye5dzd44kkatnxx2je4s2agpwtqds5lsm5mlyef7plum5danxalq2dnqap');
  static Puzzlehash catAssetId = Puzzlehash.fromHex('625c2184e97576f5df1be46c15b2b8771c79e4e6f0aa42d3bfecaebe733f4b8c');

  SimulatorHttpRpc simulatorHttpRpc;
  ChiaFullNodeInterface get fullNodeInterface => ChiaFullNodeInterface(simulatorHttpRpc);

  Future<void> farmCoins() async {
    // farm coins to testAddress
    await simulatorHttpRpc.farmTransactionBlock(testAddress);

    // move to next block, where testAddress will be rewarded with coins
    await _moveToNextBlock();
  }

  Future<List<Coin>> getCoins() async {
    final coins = await fullNodeInterface.getCoinsByPuzzleHashes([testAddress.toPuzzlehash()]);
    return coins;
  }

  Future<void> mintCats() async {
    const coinSpendsJson = '[{"coin": {"parent_coin_info": "0xe3b0c44298fc1c149afbf4c8996fb92400000000000000000000000000000001", "puzzle_hash": "0x0b7a3d5e723e0b046fd51f95cabf2d3e2616f05d9d1833e8166052b43d9454ad", "amount": 1750000000000}, "puzzle_reveal": "", "solution": ""}, {"coin": {"parent_coin_info": "0x26081b15441311d9a207a078b650a05766975814fd5aa6935a759ddaf2a05af0", "puzzle_hash": "0x1c2121e077ca2d57e3a447718fa42c3064bdf13bc7bf07bb3f39d084beb8c23e", "amount": 10000}, "puzzle_reveal": "", "solution": ""}]';
    var l = jsonDecode(coinSpendsJson) as Iterable;
    final spends = List<CoinSpend>.from(l.map<CoinSpend>((dynamic model)=> CoinSpend.fromJson(model as Map<String, dynamic>)));
    final aggSig = JacobianPoint.fromBytesG2(const [128, 115, 176, 31, 102, 152, 209, 254, 244, 249, 12, 51, 40, 145, 135, 253, 133, 60, 146, 181, 55, 110, 207, 137, 54, 6, 182, 38, 140, 113, 163, 98, 203, 20, 200, 80, 208, 99, 183, 82, 32, 123, 151, 155, 206, 65, 73, 74, 9, 25, 201, 75, 249, 211, 243, 225, 222, 89, 88, 5, 214, 137, 183, 181, 10, 33, 31, 14, 30, 189, 53, 116, 255, 135, 202, 139, 216, 120, 57, 159, 76, 68, 123, 160, 78, 205, 189, 48, 2, 7, 241, 77, 192, 129, 61, 196]);
    final spendBundle = SpendBundle(coinSpends: spends, aggregatedSignature: aggSig);
    try {
      await fullNodeInterface.pushTransaction(spendBundle);
    } on DoubleSpendException {
      throw Exception('Can only mint cats one time in a simulator session');
    }
    

    // move to next block, where testAddress will receive cats
    await _moveToNextBlock();
  }

  Future<List<CatCoin>> getCatCoins() async {
    final outerPuzzleHash = WalletKeychain.makeOuterPuzzleHash(testAddress.toPuzzlehash(), catAssetId);
    final catCoins = await fullNodeInterface.getCatCoinsByOuterPuzzleHashes([outerPuzzleHash]);
    return catCoins;
  }

  Future<void> _moveToNextBlock() async {
    await simulatorHttpRpc.farmTransactionBlock(otherAddress);
  }
}