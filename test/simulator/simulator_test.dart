...
Future<void> main() async {
  print(path.current);
  print(path.join(path.current, 'test/simulator/temp/config/ssl/full_node/private_full_node.crt'));
  final simulatorFullNode = SimulatorHttpRpc('https://localhost:5000',
  ...
  final c = HttpClient(
     context: 
          ...
          ..usePrivateKeyBytes(File(path.join(path.current, 'test/simulator/temp/config/ssl/full_node/private_full_node.key')).readAsBytesSync())
          ..useCertificateChainBytes(File(path.join(path.current, 'test/simulator/temp/config/ssl/full_node/private_full_node.crt')).readAsBytesSync())
    )
  ..badCertificateCallback = (cert, host, port) => true;

  var key = File(path.join(path.current, 'test/simulator/temp/config/ssl/full_node/private_full_node.key')).readAsBytesSync();
  print(key);
    
  // final x = await c.postUrl(Uri.parse('https://localhost:5000/get_blockchain_state'));
  // x.write(<dynamic, dynamic>{});
  // var t = await x.close();
  // final stringData = await t.transform(utf8.decoder).join();
  // print(stringData);
  // print(x.)
  final fullNodeInterface = ChiaFullNodeInterface(simulatorFullNode);
  final blockchainAwaitUtil = BlockchainAwaitUtil(simulatorFullNode);
  await simulatorFullNode.getBlockchainState();
  ...
  const address = Address('xch1pdar6hnj8c9sgm74r72u40ed8cnpduzan5vr86qkvpftg0v52jkstxap9z');
  
  await simulatorFullNode.farmTransactionBlock();
