...
Can also leave the faucet url and payload empty if you would like to manually send the XCH needed to create the PlotNFT:

```console
dart bin/chia_crypto_utils.dart Create-WalletWithPlotNFT --output-config preload.pconf --full-node-url <FULL_NODE_URL> --faucet-request-url '' --faucet-request-payload ''
```
