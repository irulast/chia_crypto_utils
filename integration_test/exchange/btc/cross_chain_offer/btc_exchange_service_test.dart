...
Future<void> main() async {
  ...
  test(
    ...
    final btcHolderPublicKey =
        exchangeService.parseSignedPublicKey(btcHolderSignedPublicKey);
  ...
  test(
    ...
    final startingClawbackAddressBalance =
        await fullNodeSimulator.getBalance([clawbackPuzzlehash]);
    ...
    await Future<void>.delayed(const Duration(seconds: 10), () async {
      await fullNodeSimulator.pushTransaction(clawbackSpendBundle);
      await fullNodeSimulator.moveToNextBlock();
      final endingClawbackAddressBalance =
          await fullNodeSimulator.getBalance([clawbackPuzzlehash]);
  ...
  test(
    ...
    final btcHolderPrivateKey = PrivateKey.fromHex(
  ...
  test(
      'should transfer XCH to escrow address and sweep funds to BTC holder using preimage',
      () async {
    final xchHolder = ChiaEnthusiast(fullNodeSimulator, walletSize: 2);
    ...
    final xchHolderPublicKey =
        exchangeService.parseSignedPublicKey(xchHolderSignedPublicKey);
    ...
    final startingSweepAddressBalance =
        await fullNodeSimulator.getBalance([sweepPuzzlehash]);
    ...
    final endingSweepAddressBalance =
        await fullNodeSimulator.getBalance([sweepPuzzlehash]);
  ...
  test(
      'should transfer XCH to escrow address and sweep funds to BTC holder using private key',
      () async {
    final xchHolder = ChiaEnthusiast(fullNodeSimulator, walletSize: 2);
    final btcHolder = ChiaEnthusiast(fullNodeSimulator, walletSize: 2);
    await xchHolder.farmCoins();
    ...
    final xchHolderPrivateKey = PrivateKey.fromHex(
      '308f34305ed545c7b6bdefe9fff88176dc3b1a68c40f9065e2cf24c98bf6a4e1',
    );
  ...
  test('exchange should fail if parties input different timeouts', () async {
    ...
    final xchHolderescrowCoins = await fullNodeSimulator
        .getCoinsByPuzzleHashes([xchHolderescrowPuzzlehash]);
    final btcHolderescrowCoins = await fullNodeSimulator
        .getCoinsByPuzzleHashes([btcHolderescrowPuzzlehash]);
  ...
  test('exchange should fail if user shares wrong signed public key', () async {
    ...
    final xchHolderWrongPrivateKey = PrivateKey.fromHex(
      '308f34305ed545c7b6bdefe9fff88176dc3b1a68c40f9065e2cf24c98bf6a4e1',
    );
